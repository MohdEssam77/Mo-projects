...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("You have to provide symbol")
        if not shares or not shares.isdigit() or int(shares) <= 0:
            return apology("You have to provide a valid number of shares")
        quote = lookup(symbol)
        if quote is None:
        ...
        cash = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=session["user_id"])[0]["cash"]
        if cash < cost:
            return apology("There is no enough cash")
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("You have to provide symbol")
        if not shares or int(shares) <= 0 or not shares.isdigit():
            return apology("You have to provide a valid number of shares")
        shares = int(shares)
        for i in stocks:
            if i["symbol"] == symbol:
                if i["total_shares"] < shares:
                    ...
                    db.execute("UPDATE users SET cash = cash + :total_sales WHERE id = :user_id", total_sales=total_sales, user_id=session["user_id"])
